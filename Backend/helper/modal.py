...
class TVShowSchema(BaseModel):
    ...
    watch_providers: List[Dict] = Field(default_factory=list, description="OTT watch providers by region")
    original_language: Optional[str] = Field(default=None, description="Original language from metadata provider")
    origin_country: List[str] = Field(default_factory=list, description="Origin countries from metadata provider")
    production_countries: List[str] = Field(default_factory=list, description="Production countries from metadata provider")
    content_tags: List[str] = Field(default_factory=list, description="Derived content category tags")
